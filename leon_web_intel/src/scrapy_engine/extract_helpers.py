...
def _hard_captcha_wall(html_lower: str, captcha_keywords: list[str]) -> bool:
    ...
    for k in captcha_keywords:
        if k.strip() and k.lower() in html_lower:
            return True
    return False
...
def access_control_triplet(
    ...
    *,
    ...
    """
    Decide paywall/login/captcha signals without naive full-page keyword scans.

    Requires main text already extracted (trafilatura): long articles pass unless hard/strong gates fire.
    """
    hl = html_text.lower()
    body_lower = (extracted_plain or "").lower()
    visible_lower = visible_text_lower(html_text)

    if _hard_captcha_wall(hl, captcha_keywords):
        return False, False, True

    if _hard_denial_wall(hl):
        return True, False, False

    pay_strong, login_strong = _strong_gate_wall(hl)
    if pay_strong or login_strong:
        return pay_strong, login_strong, False

    if content_length >= min_article_content_length:
        return False, False, False

    vis_soft_total, body_soft = _soft_keyword_pressure(body_lower, visible_lower, paywall_keywords, login_keywords)
    if body_soft or vis_soft_total >= soft_repeat_threshold:
