...
from settings import CrawlRules, KnownApiConfig
...
def decide_best_strategy(profile: SourceProfile, rules: CrawlRules) -> SourceProfile:
    """Apply the documented decision tree (known API > RSS > sitemap > HTML > JS > gates)."""
    if profile.has_known_api:
        profile.best_strategy = "api_first"
        profile.tos_risk = "low"
...
class SourceProfiler:
    def __init__(
        ...
        *,
        rules: CrawlRules,
        known_apis: KnownApiConfig,
        http: CachedHttpClient,
        db: WebIntelDB,
        raw_store,
    ) -> None:
        self.rules = rules
        self.known_apis = known_apis
        self.http = http
        self.db = db
    ...
    def profile_source(self, input_url: str) -> SourceProfile:
        ...
        try:
            ...
            api_hit = detect_known_api(norm.domain, self.known_apis)
            if api_hit:
                profile.has_known_api = True
                profile.known_api_adapter = api_hit.entry.adapter
                profile.known_api_endpoint_hint = api_hit.entry.endpoint_hint
