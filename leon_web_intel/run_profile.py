...
from collectors.api_collector import record_api_placeholder  # noqa: E402
...
def print_profile_summary(db: WebIntelDB) -> None:
    ...
    print(f"API first: {counts.get('api_first', 0)}")
    ...
    print(f"Database: {ROOT / 'data' / 'db' / 'web_intel.duckdb'}")
    print("")
        ...
        for row in profiles:
            ...
            try:
                if strategy == "api_first":
                    record_api_placeholder(
                        source_id=sid,
                        endpoint_hint=row.get("known_api_endpoint_hint"),
                        adapter=row.get("known_api_adapter"),
                        db=db,
                    )
                    continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    known = load_known_api_config(ROOT / "config" / "known_api_adapters.yaml")
